...
<div class="container">
    ...
    <span></span> <i class="fa fa-caret-down"></i>
                        ...
                        <div class="row">
                             <div class="col-md-4"><h3>Total Records <span style="background-color:#0cc285 !important;font-size:20px;padding: 5px;" class="badge"><strong class="totalrecordsdatatable">0</strong></span></h3></div>
                        </div>
                        
                          <div class="table-responsive">
                                    <table data-link="{{route('supervisorpanel.student.classall.attendance.datatable')}}" id="todayclasses-datatable"
    ...
    <div style="width:1250px !important" class="modal-dialog modal-lg">
        <form id="formlessonstudentnew" action="{{ route('supervisorpanel.student.lesson.save') }}"  method="POST" role="form">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
 ...
 "use strict";
    ...
    function cb(start, end) {
        $('#reportrange span').html(start.format('DD-MM-YYYY  hh:mm A') + ' - ' + end.format('DD-MM-YYYY  hh:mm A'));
         startdate = start.format('DD-MM-YYYY  hh:mm A');
         enddate = end.format('DD-MM-YYYY  hh:mm A');
    }
    ...
    $(document).on('click', '.btnsearchForm', function() {
     ...
     $(document).on('click','.btndeleteattendanceclick',function(){
        
        
         var attendanceid = $(this).attr('data-id');
         var route = '{{ route("supervisorpanel.student.attendance.delete") }}';

         swal({
      title: "Are you sure?",
      text: "You Want To Delete Attendance!",
      icon: "warning",
      buttons: [
        'No!',
        'Yes, I am sure!'
      ...
      if (isConfirm) {
          $.post(route, {attendanceID:attendanceid,_token:$('meta[name="csrf-token"]').attr('content')},function(res){
    ...
    var oTable = $('#todayclasses-datatable').DataTable({
     ...
     $(document).on('click', '.btneditleson', function() {
        ...
        var route = '{{ route("supervisorpanel.student.lesson.edit", ":id") }}';
