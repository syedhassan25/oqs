...
<div class="container">
    ...
    <div id="page-title">
        <h2>{{ $pageTitle }} &nbsp;&nbsp; <a class="btn btn-primary " href="{{route('supervisorpanel.task.index')}}">Back</a> <button class="btn btn-primary pull-right btncreatetask"> Create Task</button>
    ...
    <div class="panel">
        <div class="panel-body">
            @if($Task->isImportant == 1)
              <div class="ribbon">
                    <div class="bg-danger">Important</div>
                </div>
             @endif    
                             ...
                             <table 
                                    ...
                                    <tbody>
                                        @if(count($TaskMultistudent) > 0)
                                            ...
                                            <tr>
                                                    <td><a href="{{($multi_std_data->employeename) ? route('admin.teacher.edit',$multi_std_data->employeeId) : '#' }}"> {{($multi_std_data->employeename) ?  $multi_std_data->employeename   : 'no Teacher Found'}} </a></td>
                                                    <td><a href="{{route('admin.student.edit',$multi_std_data->id)}}" >{{$multi_std_data->studentname}}</a></td>
                                                    <td>{{$multi_std_data->group}}</td>
                                                    <td>{{$multi_std_data->CountryName}}</td>
                                                    <td>{{$multi_std_data->skypid_1}}</td>
                                                     <td><?php 
      ...
      <label>Description :</label>
        <textarea name="" rows="10" readonly class="textarea-autosize">{{$Task->taskDescription}}</textarea>
      ...
      <div class="row divassignusers">
         <h4>Task Created By</h4>
                    <div class="col-md-12" >
                         ...
                         <a href="#" class="badge badge-success">{{$Task->taskcreatorname}}&nbsp;|&nbsp;Created Date : {{$Task->created_at_add}}&nbsp;|&nbsp; Completed Date : {{$Task->firsttaskCompleteddate}}</a>
         ...
         <h4>Task Cancel by</h4>
                    <div class="col-md-12" >
                        ...
                        @endforeach
                           ...
                           <div style="display:{{(count($TaskReAssignhistory) > 0) ? 'block' : 'none' }}" class="row ">
                    ...
                    <div class="col-md-12" >
                         ...
                         <br/>
                          ...
                          <div style="display:{{(count($taskExtendDatehistory) > 0) ? 'block' : 'none' }}" class="row ">
               ...
               Comment
                ...
                <div class="row">
                    <div class="col-12" >
                             ...
                             <table id="commentdatatable" data-link="{{route('supervisorpanel.task.comment.datatable')}}"
                                    style="width:100% important" class="table table-bordered table-hover">
    ...
    <div id="taskAssignModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                <form id="taskReAssignForm" method="post" action="{{ route('supervisorpanel.task.ReAssignuser.store') }}"
    ...
    <div id="taskExtenddateModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-sm">
            ...
            <!-- Modal content-->
            <div class="modal-content">
                <form id="taskExtenddateForm" method="post" action="{{ route('supervisorpanel.task.extendDate.store') }}"
                    enctype="multipart/form-data">
 ...
 <input type="hidden" id="taskextenddateID"  name="id" value="" />
    ...
    <div id="taskModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <!-- Modal content-->
            <div class="modal-content">
                <form id="taskForm" method="post" action="{{ route('supervisorpanel.task.store') }}"
                    enctype="multipart/form-data">
                          ...
                          <div class="row" >
                            <div class="col-sm-6">
                                ...
                                <div class="form-group">
                                       ...
                                       <label>&nbsp;&nbsp;IsImportant <span class="m-l-5 text-danger  errorlabelmy">
                                            *</span></label>
      ...
      if (isConfirm) {
        swal({
          ...
          icon: 'success'
           ...
           $.post('{{route("supervisorpanel.task.status.detail.change")}}',{_token:token,taskstatus:status,id:taskId},function(res){
            // alert("successfully update status");
            // window.location.reload();
            
             window.location =  "{{route('supervisorpanel.task.index')}}";
...
$(document).on('click','.btncomment',function(){
        ...
        if(comment != ""){
            $.post('{{route("supervisorpanel.task.comment")}}',{_token:token,comment:comment,taskid:"{{ request()->route('id') }}"},function(){
 ...
 var oTable = $('#commentdatatable').DataTable({
    ...
    $(document).on('click','.btnAssignTo',function(){
        ...
        $.get('{{route("supervisorpanel.task.AssignUsers")}}',{id:id},function(res){
             console.log(res);
    ...
    $(document).on('submit', '#taskReAssignForm', function(e) {
        ...
        $.ajax({
                type: "POST",
                url: '{{ route("supervisorpanel.task.ReAssignuser.store") }}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
    ...
    $(document).on('click','.btnExtendDate',function(){
     ...
     $(document).on('change', '.dropdownGetUsers', function() {
        ...
        $.post('{{route("supervisorpanel.task.getUserRolewise")}}', {
            roleType: roleType,
     ...
     $(document).on('change', '#SelectGroup', function() {
        let group = $(this).val();
        $.get('{{route("supervisorpanel.task.getStudentBygroup")}}', {
            group: group,
            _token: $('meta[name="csrf-token"]').attr('content')
        }, function(res) {
            console.log(res);
     ...
     $(document).on('change', '#SelectTeacher', function() {
        ...
        $.get('{{route("supervisorpanel.task.getStudentByteacher")}}', {
            teacherid: teacherid,
            _token: $('meta[name="csrf-token"]').attr('content')
        }, function(res) {
            console.log(res);
     ...
     $(document).on('submit', '#taskForm', function(e) {
        ...
        $.ajax({
                type: "POST",
                url: '{{ route("supervisorpanel.task.store") }}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
