...
<div class="container">
    ...
    <div id="page-title">
        <h2>{{ $pageTitle }} &nbsp;&nbsp; <a class="btn btn-primary " href="{{route('admissionpanel.task.index')}}">Back</a> <button class="btn btn-primary pull-right btncreatetask"> Create Task</button>
               ...
               Comment
                ...
                <div class="row">
                    <div class="col-12" >
                             ...
                             <table id="commentdatatable" data-link="{{route('admissionpanel.task.comment.datatable')}}"
                                    style="width:100% important" class="table table-bordered table-hover">
    ...
    <div id="taskAssignModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                <form id="taskReAssignForm" method="post" action="{{ route('admissionpanel.task.ReAssignuser.store') }}"
    ...
    <div id="taskExtenddateModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-sm">
            ...
            <!-- Modal content-->
            <div class="modal-content">
                <form id="taskExtenddateForm" method="post" action="{{ route('admissionpanel.task.extendDate.store') }}"
                    enctype="multipart/form-data">
 ...
 <input type="hidden" id="taskextenddateID"  name="id" value="" />
    ...
    <div id="taskModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <!-- Modal content-->
            <div class="modal-content">
                <form id="taskForm" method="post" action="{{ route('admissionpanel.task.store') }}"
                    enctype="multipart/form-data">
      ...
      if (isConfirm) {
        swal({
          ...
          icon: 'success'
           ...
           $.post('{{route("admissionpanel.task.status.detail.change")}}',{_token:token,taskstatus:status,id:taskId},function(res){
            // alert("successfully update status");
            // window.location.reload();
            
             window.location =  "{{route('admissionpanel.task.index')}}";
...
$(document).on('click','.btncomment',function(){
        ...
        if(comment != ""){
            $.post('{{route("admissionpanel.task.comment")}}',{_token:token,comment:comment,taskid:"{{ request()->route('id') }}"},function(){
 ...
 var oTable = $('#commentdatatable').DataTable({
    ...
    $(document).on('click','.btnAssignTo',function(){
        ...
        $.get('{{route("admissionpanel.task.AssignUsers")}}',{id:id},function(res){
             console.log(res);
    ...
    $(document).on('submit', '#taskReAssignForm', function(e) {
        ...
        $.ajax({
                type: "POST",
                url: '{{ route("admissionpanel.task.ReAssignuser.store") }}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
    ...
    $(document).on('click','.btnExtendDate',function(){
     ...
     $(document).on('change', '.dropdownGetUsers', function() {
        ...
        $.post('{{route("admissionpanel.task.getUserRolewise")}}', {
            roleType: roleType,
     ...
     $(document).on('change', '#SelectGroup', function() {
        let group = $(this).val();
        $.get('{{route("admissionpanel.task.getStudentBygroup")}}', {
            group: group,
            _token: $('meta[name="csrf-token"]').attr('content')
        }, function(res) {
            console.log(res);
     ...
     $(document).on('change', '#SelectTeacher', function() {
        ...
        $.get('{{route("admissionpanel.task.getStudentByteacher")}}', {
            teacherid: teacherid,
            _token: $('meta[name="csrf-token"]').attr('content')
        }, function(res) {
            console.log(res);
     ...
     $(document).on('submit', '#taskForm', function(e) {
        ...
        $.ajax({
                type: "POST",
                url: '{{ route("admissionpanel.task.store") }}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
