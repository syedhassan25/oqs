...
<div class="container">
    ...
    <div id="page-title">
        <h2>{{ $pageTitle }} &nbsp;&nbsp; <a class="btn btn-primary " href="{{route('demonstrationpanel.task.index')}}">Back</a> <button class="btn btn-primary pull-right btncreatetask"> Create Task</button>
                                    ...
                                    <tbody>
                                        @if(count($TaskMultistudent) > 0)
                                            ...
                                            <tr>
                                                    ...
                                                    <td><a href="{{route('demonstrationpanel.student.editnewform',$multi_std_data->id)}}" >{{$multi_std_data->studentname}}</a></td>
               ...
               Comment
                ...
                <div class="row">
                    <div class="col-12" >
                             ...
                             <table id="commentdatatable" data-link="{{route('demonstrationpanel.task.comment.datatable')}}"
                                    style="width:100% important" class="table table-bordered table-hover">
    ...
    <div id="taskAssignModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                <form id="taskReAssignForm" method="post" action="{{ route('demonstrationpanel.task.ReAssignuser.store') }}"
    ...
    <div id="taskExtenddateModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-sm">
            ...
            <!-- Modal content-->
            <div class="modal-content">
                <form id="taskExtenddateForm" method="post" action="{{ route('demonstrationpanel.task.extendDate.store') }}"
                    enctype="multipart/form-data">
 ...
 <input type="hidden" id="taskextenddateID"  name="id" value="" />
    ...
    <div id="taskModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <!-- Modal content-->
            <div class="modal-content">
                <form id="taskForm" method="post" action="{{ route('demonstrationpanel.task.store') }}"
                    enctype="multipart/form-data">
...
$(document).on('click', '.btnstudentdetail', function() {
        ...
        var route = '{{ route("demonstrationpanel.student.detail", ":id") }}';
...
$(document).on('click','.btnchangeStatus',function(){
    ...
    swal({
      ...
      if (isConfirm) {
        ...
        $.post('{{route("demonstrationpanel.task.status.detail.change")}}',{_token:token,taskstatus:status,id:taskId},function(res){
            // alert("successfully update status");
            // window.location.reload();
            
             window.location =  "{{route('demonstrationpanel.task.index')}}";
...
// $(document).on('click','.btncomment',function(){
 ...
 $(document).on('submit', '#commentForm', function(e) {
        ...
        // if(comment != 0){
        //     $.post('{{route("demonstrationpanel.task.comment")}}',{_token:token,comment:comment,taskid:"{{ request()->route('id') }}"},function(){
        ...
        $.ajax({
                type: "POST",
                url: '{{route("demonstrationpanel.task.comment")}}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
 ...
 var oTable = $('#commentdatatable').DataTable({
    ...
    $(document).on('click','.btnAssignTo',function(){
        ...
        let id  = $(this).attr('data-id');
        $.get('{{route("demonstrationpanel.task.AssignUsers")}}',{id:id},function(res){
             console.log(res);
    ...
    $(document).on('submit', '#taskReAssignForm', function(e) {
        ...
        $.ajax({
                type: "POST",
                url: '{{ route("demonstrationpanel.task.ReAssignuser.store") }}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
    ...
    $(document).on('click','.btnExtendDate',function(){
     ...
     $(document).on('change', '.dropdownGetUsers', function() {
        ...
        $.post('{{route("demonstrationpanel.task.getUserRolewise")}}', {
            roleType: roleType,
     ...
     $(document).on('change', '#SelectGroup', function() {
        let group = $(this).val();
        $.get('{{route("demonstrationpanel.task.getStudentBygroup")}}', {
            group: group,
            _token: $('meta[name="csrf-token"]').attr('content')
        }, function(res) {
            console.log(res);
     ...
     $(document).on('change', '#SelectTeacher', function() {
        ...
        $.get('{{route("demonstrationpanel.task.getStudentByteacher")}}', {
            teacherid: teacherid,
            _token: $('meta[name="csrf-token"]').attr('content')
        }, function(res) {
            console.log(res);
     ...
     $(document).on('submit', '#taskForm', function(e) {
        ...
        $.ajax({
                type: "POST",
                url: '{{ route("demonstrationpanel.task.store") }}',
                dataType: 'json',
                processData: false,
                contentType: false,
                cache: false,
